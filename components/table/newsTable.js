...
export default function NewsTable({setTitle, setIdToEdit, setSummary, setEditActive, setEditorContent, setExpirationDate, setAccount, setOpen}) {
...
//------------- fetch API down -----------------------------
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await fetch('/api/news-feed', {
        ...
        body: JSON.stringify({ operation: 'newsList' })
      });
  ...
  const columnsNamesMainList = [
    ...
    { key: 'expiration_date', label: 'expirace', sorting: true },
...
//-------------DELETE API down   ------------------------------------
  ...
  const handleDel = async (id) => {
    let confirmDel = confirm(`opravdu chcete smazat news feed ?`)
    if(!confirmDel){
      return;
    }
    setDisabled(true)
...
//-------------DELETE API UP   --------------------------------------
  ...
  const handleNewsPropsEdit = (idNewsToEdit) =>{
    let tempId = idNewsToEdit
    let tempRow = rows.find(row => row.id === tempId)
    let tempExpDate = new Date(tempRow.expiration_date).toISOString().split('T')[0];
    setTitle(tempRow.title)
    setSummary(tempRow.description)
    setEditorContent(tempRow.clanek)
    setExpirationDate(tempExpDate)
    setAccount(tempRow.account)
    setEditActive(true)
    setOpen(false)
    setIdToEdit(tempId)
            ...
            {paginatedRows.map((row) => (
              <React.Fragment key={row.id}>
                <tr className="text-xs md:text-sm border-b text-start dark:bg-gray-300 dark:text-white dark:even:bg-gray-200 even:bg-zinc-100 odd:bg-white dark:hover:bg-gray hover:bg-gray-50">
                  ...
                  <td className="py-2 md:mx-2 md:px-2 border-[1px] text-gray-800 text-xs md:text-sm max-w border-gray-300 whitespace-normal">
                    <button disabled={disabled} onClick={() => handleNewsPropsEdit(row.id)}>
                      <CiEdit
                        className={`h-7 w-7 hover:cursor-pointer ${
